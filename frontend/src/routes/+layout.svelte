<script>
  ...
  // No necesitamos 'fonts' aquí ahora
</script>
...
<svelte:head>
  ...
  <style>
    ...
    /* Base body styles - SIN font-family aquí */
    body {
...
<div class="font-body text-text bg-background flex min-h-screen flex-col transition-colors duration-300">
  <header class="bg-background/80 dark:bg-background/90 sticky top-0 z-50 w-full border-b border-primary/20 dark:border-primary/30 backdrop-blur-sm transition-colors duration-300">
    <nav class="mx-auto flex max-w-7xl items-center justify-between p-4">
      <a href="/" class="font-headings text-text text-2xl font-bold transition-opacity hover:opacity-80 focus:outline-none focus:ring-2 focus:ring-primary focus:ring-offset-2 rounded-sm">
        {brand.identity.name}
      </a>
      <div class="flex items-center space-x-4">
        <a href="/" class="text-text hidden text-sm font-medium transition-colors hover:text-primary focus:text-primary focus:outline-none sm:block">
          Tienda
        </a>
        <a href="/checkout" class="text-text relative flex items-center rounded-full p-1.5 transition-colors hover:bg-gray-200 dark:hover:bg-gray-700 focus:outline-none focus:ring-2 focus:ring-primary focus:ring-offset-2" aria-label="Ver carrito">
          <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="h-5 w-5">
            <path stroke-linecap="round" stroke-linejoin="round" d="M2.25 3h1.386c.51 0 .955.343 1.087.835l.383 1.437M7.5 14.25a3 3 0 00-3 3h15.75m-12.75-3h11.218c.121.001.241.015.358.043a5.25 5.25 0 01-4.256 5.207H3.375a.75.75 0 01-.748-.686c-.03-1.667.988-3.033 2.67-3.268m11.438-6.75H5.625m13.5 0v1.149c0 .416-.16.816-.44 1.121l-4.04 4.041a.75.75 0 01-1.06 0l-4.041-4.041A1.875 1.875 0 015.625 8.649V7.5H19.5z" />
          </svg>
          {#if itemCount > 0}
            <span class="bg-accent absolute -top-1 -right-1 flex h-4 w-4 items-center justify-center rounded-full text-[10px] font-bold text-white">{itemCount}</span>
          {/if}
        </a>
        <ThemeToggle />
      </div>
